# Es wird Verspätung gerechnet.
# Negative Verspätung heisst, das Fahrzeug ist zu früh an
# der Station angekommen.

# leu$versp <- leu$ist_an_von - leu$soll_an_von
# kum_versp <- tapply(leu$versp, leu$betriebsdatum, FUN=sum)

library(data.table)
d <- read.csv("../99_data/linie10richtung1.dat", header=T)
d <- d[c("betriebsdatum", "kurs", "halt_kurz_von1",
 "soll_an_von", "ist_an_von")]

stationen_linie_10 <- c("BABH07", "BERN", "BGLA07", "BOEO",
		"BPLA", "CENT", "ETHZ", "FLUG07",
		"FRAF07", "GGLA07", "GLIN07", "HALB",
		"HEGG",
		"HIRS", "IRCH", "KINK", "LANM",
		"LETZ", "LEUT", "MIBU", "RBAU07",
		"RIGI", "SALE", "SOER", "UNTR07",
		"WINK", "ZOER")

d <- as.data.table(d)
d$betriebsdatum <- as.Date(d$betriebsdatum, format="%d.%m.%y")

# ****************************************
# Note: Which course should be considered?
# ****************************************
d <- d[d$kurs==5]
d <- d[d$halt_kurz_von1 %in% stationen_linie_10]

d$versp <- d$ist_an_von - d$soll_an_von

# Omit too large delays.
d <- d[abs(d$versp)<3000]



library(ggplot2)

# Font size of axis adjusted to png file output.
# png("./014-vbz.png", width=1000, height=1500)
# print(g)
# dev.off()

g <- ggplot(d, aes(betriebsdatum, versp))
g <- g + geom_line() + facet_wrap(~halt_kurz_von1)

# 'hjust=1': Set axis label to right justified such that
#            it doesn't overwrite on top of the plotting area.
g <- g + theme(axis.text.x = element_text(angle=90, size=24,
                                          hjust=1),
               axis.title.x = element_text(size=32),
               axis.title.y = element_text(size=32),
               axis.text.y = element_text(size=24))

# Only the visualization of the axis is clipped.
# g <- g + ylim(-2000, 2000)

g <- g + theme(strip.text.x = element_text(size=24))
g <- g + xlab("Date of Operation")
g <- g + ylab("Total Delay/d [s]")
g <- g + scale_x_date(date_breaks="4 months", date_labels="%d.%m.%y")

print(g)

